...
> 版本：Agent Skill 运行版 v1.7（2026-08-26）
>
> 原则：自由探索、明确判定、因果连续、正典校准、持久化与快节奏；游戏语义、工作区状态和视觉生成各有独立职责。
...
| [presentation.md](presentation.md) | 公开面板、触发词、输出编排、插图与表现层故障恢复 |
| [appendices.md](appendices.md) | 术语、物品、判例、开场模板与维护来源 |

工作区存储和视觉生成分别由 [runtime-and-storage.md](runtime-and-storage.md) 与 [visual-media.md](visual-media.md) 定义。它们不得重新定义游戏语义。
...
其他常规回合按 [runtime-core.md](runtime-core.md) 的升级条件完整读取相关权威模块。当前上下文中仍可访问且匹配 `ruleset_digest` 的完整文本可以复用；仅有保存标记或旧会话记忆时仍视为未加载。
...
- 生成状态面板、提示、插图或编排回合输出：读取 [presentation.md](presentation.md)。
- 解释术语、核对判例或维护来源：读取 [appendices.md](appendices.md)。
